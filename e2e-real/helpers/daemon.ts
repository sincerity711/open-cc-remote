...
function mkStateDir(daemon_id: string): string {
...
export async function startDaemon(opts: DaemonOpts): Promise<DaemonHandle> {
  const state_dir = opts.state_dir ?? mkStateDir(opts.daemon_id);
  ...
  const handle: DaemonHandle = {
    ...
    async stop() {
      ...
      // Cleanup state dir unless the user supplied it.
      if (opts.state_dir === undefined) {
